...
function iniciarMapa() {
  ...
  function actualizarBarra(valor, maximo, minimo, anchoTotal) {
    ...
    const intensidad = Math.round(255 - porcentaje * 155); // 255 a 100
    const color = `rgb(${intensidad}, ${Math.round(intensidad * 0.5)}, 0)`;
  
    // Debug
    console.log(`Valor: ${valor}, Normalizado: ${porcentaje}, Ancho en px: ${ancho}`);
  
    // Actualizar el ancho del rectángulo
    document.getElementById("barra-relleno").style.transition = "width 0.6s ease";
    document.getElementById("barra-relleno").setAttribute("width", ancho);
    document.getElementById("barra-relleno").setAttribute("fill", color);
  }
